...
void strassen_omp(const std::vector<double>& a,
                 const std::vector<double>& b,
  ...
  const int h = m1 / 2;
  const int hh = h * h;
  std::vector<double> a11(hh);
  std::vector<double> a12(hh);
  std::vector<double> a21(hh);
  std::vector<double> a22(hh);
  std::vector<double> b11(hh);
  std::vector<double> b12(hh);
  std::vector<double> b21(hh);
  std::vector<double> b22(hh);
  std::vector<double> r11(hh);
  std::vector<double> r12(hh);
  std::vector<double> r21(hh);
  std::vector<double> r22(hh);
  std::vector<double>  p1(hh);
  std::vector<double>  p2(hh);
  std::vector<double>  p3(hh);
  std::vector<double>  p4(hh);
  std::vector<double>  p5(hh);
  std::vector<double>  p6(hh);
  std::vector<double>  p7(hh);
...
#pragma omp section
    {
      std::vector<double> t1(hh);
      std::vector<double> t2(hh);
...
#pragma omp section
  ...
  std::vector<double> t3(hh);
  std::vector<double> t4(hh);
  sumMatrix(p1, p3, &t3);
  sumMatrix(t3, p6, &t4);
...
#pragma omp parallel for default(shared)
  for (int i = 0; i < h; i++) {
    for (int j = 0; j < h; j++) {
      const int tmp = i * h + j;
      result->at(i * n + j) = r11[tmp];
      result->at(i * n + j + h) = r12[tmp];
      result->at((i + h) * n + j) = r21[tmp];
      result->at((i + h) * n + j + h) = r22[tmp];
    }
  }
}
